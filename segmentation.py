...
def clahe_enhancement(gray, clip_limit=2.0, tile_grid_size=(12, 12)):
    ...
    # 保存增强结果用于调试
    cv2.imwrite('clahe_enhancement_debug.png', enhanced)
...
def coarse_segmentation(enhanced):
    ...
    # 显示中间结果用于调试
    cv2.imwrite('coarse_segmentation_debug.png', binary)
    cv2.imwrite('chest_mask_debug.png', chest_mask)
...
def locate_lung_regions(binary):
    ...
    # 保存潜在肺区域用于调试
    cv2.imwrite('potential_lung_regions_debug.png', potential_lung_mask)
    ...
    # 保存左右肺掩码用于调试
    cv2.imwrite('left_lung_mask_debug.png', left_lung_mask)
    cv2.imwrite('right_lung_mask_debug.png', right_lung_mask)
...
def refine_segmentation(enhanced, lung_mask, bbox):
    ...
    # 保存精细分割结果用于调试
    cv2.imwrite('refined_segmentation_debug.png', refined)
...
def morphological_optimization(lung_mask):
    ...
    # 保存形态学优化结果用于调试
    cv2.imwrite('morphological_optimization_debug.png', opened)
...
def repair_heart_boundary(lung_mask):
    ...
    try:
        ...
        if contours:
            ...
            # 保存边界修复结果用于调试
            cv2.imwrite('heart_boundary_repair_debug.png', result)
...
def optimize_with_morphology_and_connectivity(thresh, chest_mask):
    ...
    # 保存中间结果用于调试
    cv2.imwrite('morphology_opening_debug.png', opening)
    cv2.imwrite('candidate_mask_debug.png', candidate_mask)
    cv2.imwrite('morphology_closing_debug.png', closing)
...
def main(image_path):
    try:
        ...
        # 保存金标准用于调试
        cv2.imwrite('ground_truth_debug.png', ground_truth)
        ...
        print("已保存增强方法对比图: enhancement_comparison.png")
        ...
        print(f"Dice系数: {dice_score:.4f}")

        # 可视化结果
        visualize_results(original, clahe, final_segmentation, ground_truth, dice_score)
        print("已保存分割结果对比图: segmentation_results.png")

        # 保存最终分割结果
        cv2.imwrite('final_segmentation.png', final_segmentation)
        print("已保存最终分割结果: final_segmentation.png")

        # 保存中间结果对比
        debug_visualization(original, clahe, binary, left_lung_mask, right_lung_mask, final_segmentation, ground_truth)
...
if __name__ == "__main__":
    # 使用指定的图像路径
    image_path = "MontgomerySet/CXR_png/MCUCXR_0001_0.png"
    main(image_path)